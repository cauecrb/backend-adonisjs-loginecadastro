...
class ProdutoController {
  ...
  async index ({ request, response, view }) {
    const prodTot = await Produtos.all();
  ...
  async store ({ request, response }) {
    const data = request.only(['valor', 'produto', 'quantidade']);

    const guardProd = await Produtos.create(data);
  ...
  async show ({ params, request, response, view }) {
    const prod = await Produtos.findOrFail(params.id);
  ...
  async destroy ({ params, request, response }) {
    const prod = await Produtos.findOrFail(produto.id);
